...
public class RoleController {
    ...
    public HttpEntity<?> getRoles(@ApiIgnore @CurrentUser Teacher teacher){
            return (teacher.getRole().equals("SUPERADMIN")) ? ResponseEntity.status(202).body(roleRepository.findAll()) : ResponseEntity.status(409).body("Not found");
    }
    ...
    public HttpEntity<?> getRoleById(@PathVariable Long roleId, @ApiIgnore @CurrentUser Teacher teacher){
        return (teacher.getRole().equals("SUPERADMIN")) ? ResponseEntity.status(202).body(roleRepository.findById(roleId)) : ResponseEntity.status(409).body("Not found");
    }
